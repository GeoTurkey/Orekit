...
public class MeanLocalSolarTime extends AbstractSKControl {
     ...
     * @param ascending
     *            if true, solar time is computed when crossing the specified latitude from south to
     *            north
     * @param solarTime
     *            target solar time (in fractional hour, i.e 9h30 = 9.5)
     * @param solarTimetolerance
     *            solar time tolerance (in hours)
     * @param horizon
     *            time horizon duration
     * @param compensateLongBurn
     *            if true, long burn inefficiency should be compensated
     * @exception OrekitException
     *                if the UTC-TAI correction cannot be loaded
     */
    public MeanLocalSolarTime(final String name,
                              final String controlledName,
                              final int controlledIndex,
                              final TunableManeuver model,
                              final double firstOffset,
                              final int maxManeuvers,
    ...
    public ScheduledManeuver[] tuneManeuvers(final ScheduledManeuver[] tunables,
                                             final BoundedPropagator reference) throws OrekitException {
        ...
        if (forceReset || (mst[2] < 0 && extremum > hMax)) {
            ...
            newHdot = FastMath.copySign(FastMath.sqrt(4 * mst[2] * (mst[0] - targetH)), (tPeak > 0) ? mst[1] : -mst[1]);
        }
        ...
        for (final ScheduledManeuver maneuver : tuned) {
            ...
            AdapterPropagator.DifferentialEffect directEffect = new SmallManeuverAnalyticalModel(state, maneuver.getDeltaV(), maneuver.getIsp());
            AdapterPropagator.DifferentialEffect resultingEffect = new J2DifferentialEffect(state, directEffect, false, referenceRadius, mu, j2);
     ...
     *             if state cannot be propagated around maneuvera
     */
    private ScheduledManeuver longBurnCompensation(final ScheduledManeuver maneuver) throws PropagationException {
        ...
        return new ScheduledManeuver(maneuver.getModel(), maneuver.getDate(), maneuver.getDeltaV(), maneuver.getThrust(), reductionFactor
                                                                                                                          * getModel().getCurrentISP(), maneuver.getTrajectory(), maneuver.isReplanned());
    }
